...
我们以这两个过程对Unix网络编程中的5中IO操作进行划分解读
...
- 多路复用 I/O multiplexing  (select & poll)
   ...
   多路复用可以理解为NIO的多线程实现，所以也是同步非阻塞。

   每条线程有一个多路复用器（Selector）将多个Socket注册在一个Selector上，因此一条线程具备处理多个网络请求的能力，多路复用是Reactor线程的核心，后文会具体阐述。
...
多路复用实现select，pselect，poll，epoll，kqueue，，接下来分别看下这几种多路复用的具体实现以及区别
...
poll不受fd数量显示，因为select以位图计算，因而处理fd数量已经限定，poll传入的是set
...
epoll支持水平触发（LT，level trigger）和边缘触发（ET，edge trigger）两种模式
...
可以理解为LT水平触发，只要有数据可以读，不管怎样都会通知。而ET为边缘触发，只有状态发生变化时才会通知，可以理解为电平变化。（优缺点）
...
从之前的一片技术文章猜测epoll性能要好于kqueue，个人没有去做过比较和验证，但是这个还是依赖于操作系统

### 小结

多路复用具备了仅用一条线程处理大量请求的能力，

![多路复用](images/多路复用.png)

![多线程](images/多线程.png)

再从数据读取的四个过程过程看这几种多路复用实现的区别
...
​	由于多路复用属于系统调用，依赖于操作系统，linux内核2.6版本之后的开始支持epoll调用，而mac os则仅支持Kqueue，windows支持ICOP，JDK在启动的时候会根据操作系统自动选择选用哪种多路复用的实现，具体如下：
...
除此之外，netty针对以下两个特定平台进行优化

- Linux (4.0.16 起)
- MacOS/BSD (4.1.11 起)

落实到IO多路复用即Epoll以及Kqueue实现进行优化，主要涉及如下几个类
...
当然需要引入额外的依赖，工程应用具体配置参见[Native transports](http://netty.io/wiki/native-transports.html)

与基于NIO的传输相比，这些JNI传输添加特定于特定平台的功能，产生更少的垃圾，提高性能，具体如下：
...
[Native transports](http://netty.io/wiki/native-transports.html)